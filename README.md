...
3. Press `a` to add a new segment from the ones not yet enabled, `d` to remove the highlighted one, `n` to split the statusline onto a new line right after the highlighted segment, or `r` to reset to the defaults.

   ![Add Segment Picker](assets/add_segment.png)
   This is the add segment image.
