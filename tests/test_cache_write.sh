#!/bin/bash
# Verifies cache_write shows the creation token count with its % share of total context.
set -euo pipefail
cd "$(dirname "$0")/.."

TMP_CONFIG=$(mktemp)
echo '{"segments":["cache_write"]}' > "$TMP_CONFIG"
...
if [[ "$output" != *"Cache Write: 400 (20%)"* ]]; then
    echo "FAIL: expected 'Cache Write: 400 (20%)', got: $output"
    rm -f "$TMP_CONFIG"
    exit 1
fi
echo "PASS: cache_write shows token count and percentage"
...
if [[ -n "$output_zero" ]]; then
    echo "FAIL: expected empty output when cache_creation_input_tokens=0, got: $output_zero"
    rm -f "$TMP_CONFIG"
    exit 1
fi
echo "PASS: cache_write hides when zero"
